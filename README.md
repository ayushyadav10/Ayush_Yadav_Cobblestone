...
* **Non-linear Transforms & Interaction Terms (3 features):** `residual_load_lag24h_squared` and relative `renewable_penetration_lag24h` and `ttf_roll_mean_7d`.
...
*Note: The scores below represent results from the latest execution. Small variations ($\pm 0.05$ EUR/MWh) may occur on different systems due to underlying package versions and hardware architectures.*
...
* **Spread Triggers & Positioning Signals:** Compares daily model fair-value forecasts against the frozen prompt base. Spreads exceeding $\pm 5$ EUR/MWh trigger directional LONG/SHORT position signals, while absolute variations exceeding $8$ EUR/MWh and $15$ EUR/MWh automatically map growing MEDIUM/HIGH strategy confidence level tiers.
